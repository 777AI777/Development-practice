...
│   ├── rules/                # パス別詳細ルール（api, figma-make, orchestration等）
...
SHOW_STATE_SCREENS      // UI状態デモ画面（production以外で自動ON）
```

環境変数は下記 Environment Variables セクションを参照。
...
- Mock認証（Phase1）: LocalStorage `okiny:session:userId` + `okiny:session:displayNameMap`
...
| `NEXT_PUBLIC_ENABLE_SNS_EXPANSION` | Phase2 SNS機能の表示切替 | `false` |
| `NEXT_PUBLIC_SHOW_STATE_SCREENS` | UI状態デモ画面の表示切替 | production以外で自動ON |
