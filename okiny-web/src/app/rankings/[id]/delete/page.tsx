...
export default function DeleteRankingPage() {
  ...
  const runDelete = async () => {
    ...
    if (rankingId === DEMO_RANKING_ID) {
      pushToast({ type: "info", message: "Demo ranking is not deletable." });
    ...
    try {
      ...
      pushToast({ type: "success", message: "Ranking deleted." });
      router.push("/rankings");
    } catch (error: unknown) {
      const message =
        error instanceof PublishedApiError ? error.message : "Failed to delete ranking.";
  ...
  return (
    <AppShell
      ...
      subtitle="This screen corresponds to mock 08 delete confirmation."
    >
      <div className="space-y-4">
        <p className="rounded-md border border-red-200 bg-red-50 px-4 py-3 text-sm font-semibold text-red-700">
          Deleting this ranking cannot be undone.
        </p>
