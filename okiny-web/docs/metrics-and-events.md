...
| `ranking_create_start` | 作成フォーム表示時 | `user_id`, `source` (`list`,`draft`) |
...
以下はPhase2（SNS化）で追加予定。現行のPhase1には含まれない。

| Event | 発火タイミング |
|---|---|
| `return_visit_d1` | D1再訪判定時 |
| `feed_open` | Feed表示時 |
| `reaction_sent` | Like/Save実行時 |
